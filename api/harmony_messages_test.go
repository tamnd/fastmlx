...
func TestExtractHarmonyMessagesParity(t *testing.T) {
	raw, err := os.ReadFile(filepath.Join("testdata", "extract_harmony_messages.json"))
	if err != nil {
	...
	for i, c := range cases {
		...
		got := jval{kind: kindArray, arr: ExtractHarmonyMessages(in.arr)}
...
func BenchmarkExtractHarmonyMessages(b *testing.B) {
	b.ReportAllocs()
	in, _ := parseOrdered(`[` +
	...
	for b.Loop() {
		_ = ExtractHarmonyMessages(in.arr)
	}
}
